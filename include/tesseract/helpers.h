...
// debug to file
#define MAZLOG(str, file) {\
FILE* f = NULL; \
if (file == NULL) { \
  f = fopen("MAZOUT.TESS.txt", "a+"); \
}else { \
  f = fopen(file, "a+"); \
} \
  fprintf(f, str); \
...
// debug to file
#define MAZLOG1(str, param, file) {\
  char buf[512] = {0}; \
  sprintf(buf, str, param); \
  MAZLOG(buf, file) \
}

#define MAZLOG2(str, param1, param2,  file) {\
  char buf[512] = {0}; \
  sprintf(buf, str, param1, param2); \
  MAZLOG(buf, file) \
}

#define MAZLOG3(str, param1, param2, param3,  file) {\
  char buf[512] = {0}; \
  sprintf(buf, str, param1, param2, param3); \
  MAZLOG(buf, file) \
}
